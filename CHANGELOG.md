...
- **⭐ Избранное** — вторая кнопка в chat-клавиатуре рядом с «📋 Команды»:
  - Показывает inline-кнопки выбранных пользователем команд
  - Настройка через **⚙️ Настройка** → пронумерованный список всех команд
  ...
  - Команды располагаются в порядке, заданном пользователем
- **Вложенное меню команд** — полная переработка `commands_menu.py`:
  - Уровень 0: Chat-клавиатура [📋 Команды] → TOC (Оглавление)
  - Уровень 1: TOC с кнопками категорий → страница категории
  - Уровень 2: Страница категории (6 команд + навигация) → выбор → `/cmd` в LLM
- **Payload-маршрутизация** — text-кнопки с `payload={"cmd": "..."}` перехватываются до LLM (0 токенов)
- **Авто-отправка клавиатуры** для новых peer (один раз при первом сообщении)
- **YOLO mode** — режим полного авто-одобрения всех входящих сообщений

### Changed

- `adapter.py`: полная интеграция `MenuHandler` через `_process_update` и `_process_callback_event`
- `commands_menu.py`: переписан с нуля — v1 → v2 (709 строк, 4 категории, 65 команд)
